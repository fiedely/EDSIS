...
import { X, Plus, FileText, RefreshCw, Settings, Shield, User, LogOut } from 'lucide-react';

interface SidebarProps {
  isOpen: boolean;
  onClose: () => void;
  onAddItem: () => void;
  userRole?: string; // For future RBAC
}
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, onAddItem, userRole = 'admin' }) => {
  // Mock Permissions Logic (We will connect this to real Auth later)
  ...
  return (
    <>
      ...
      <div className={`fixed inset-y-0 left-0 w-72 bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-out flex flex-col ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        {/* User Profile Snippet (Placeholder) */}
        ...
        <div className="flex-grow overflow-y-auto py-4">
            ...
            <MenuButton icon={<FileText size={18} />} label="Export List" onClick={() => {}} />
